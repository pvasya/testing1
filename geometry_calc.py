...
def eq4_to_ABC(a, b):
    ...
    if a == 0 or b == 0:
        raise ValueError(f"a = 0 or b = 0. Set non-zero integer values for a and b in Line 1.")
    A = b
    B = a
    C = -a * b
...
def intersect_two_lines(A1, B1, C1, A2, B2, C2):
    ...
    if approx_eq(A1 * C2, A2 * C1) and approx_eq(B1 * C2, B2 * C1):
...
def unique_points(points):
    ...
    for p in points:
        found = False
        for q in unique:
            if approx_eq(p[0], q[0]) and approx_eq(p[1], q[1]):
                found = True
                break
        if not found:
            unique.append(p)
